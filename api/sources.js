...
async function handler(request, context) {
  try {
    ...
    if (request.method === 'GET') {
      ...
      // FIX: use s.name (not s.title) and s.feed_url for RSS feed column
      const result = await query(
    ...
    if (request.method === 'POST') {
      ...
      // Accept both {title, url, feedUrl} and legacy {title, url} formats
      const { title, url, feedUrl, feed_url } = body;
      const rssFeedUrl = feedUrl || feed_url || url;
      const websiteUrl = (url !== rssFeedUrl ? url : null) || rssFeedUrl;
      ...
      try {
        // FIX: insert into name + url (website) + feed_url (RSS) — not title
        const result = await query(
        ...
        if (error.code === '23505') {
          return new Response(JSON.stringify({ error: 'Source feed URL already exists' }), { status: 409, headers: { 'Content-Type': 'application/json' } });
        }
        throw error;
      }
    }
    ...
    if (request.method === 'PUT') {
      ...
      // FIX: update column 'name', not 'title'
      ...
      if (!result.rows.length) return new Response(JSON.stringify({ error: 'Source not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
      return new Response(JSON.stringify(result.rows[0]), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (request.method === 'DELETE') {
      ...
      try {
        ...
        if (!result.rows.length) return new Response(JSON.stringify({ error: 'Source not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
        return new Response(JSON.stringify({ deleted: true, id: result.rows[0].id }), { status: 200, headers: { 'Content-Type': 'application/json' } });
      } catch (txError) {
