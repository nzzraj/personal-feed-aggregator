...
async function handler(request) {
  ...
  try {
    ...
    const result = await query(
      `SELECT a.id, a.title, a.url, a.excerpt, a.pub_date, a.read, a.saved, a.read_time_minutes,
        s.id as source_id, s.name as source_title,
        CASE WHEN a.title ILIKE $1 THEN 2 ELSE 1 END as relevance
      FROM articles a JOIN sources s ON a.source_id = s.id
      WHERE a.title ILIKE $1 OR a.excerpt ILIKE $1
      ORDER BY relevance DESC, a.pub_date DESC LIMIT $2 OFFSET $3`,
      [`%${searchQuery}%`, limit, offset]
    );
    return new Response(JSON.stringify({ results: result.rows, query: searchQuery, count: result.rows.length }), {
